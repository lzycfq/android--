...
(function(i, ha, t, p) {
	var l = function(h) {
		...
		function H(a) {
			var b = L(a, "aoPreDrawCallback", "preDraw", [a]);
			if(i.inArray(false, b) !== -1) O(a, false);
			else {
				var c, d;
				b = [];
				var e = 0,
				...
				if(a.aiDisplay.length !== 0) {
					...
					else if(c.sEmptyTable && a.fnRecordsTotal() === 0) f = c.sEmptyTable;
			...
			for(var c = a.nTableWrapper, d = a.sDom.split(""), e, f, g, j, m,
				...
				if(g == "<") {
					...
					if(m == "'" || m == '"') {
						...
						if(k == "H") k = a.oClasses.sJUIHeader;
						else if(k == "F") k = a.oClasses.sJUIFooter;
						if(k.indexOf(".") != -1) {
							m = k.split(".");
		...
		function Da(a, b) {
			...
			else M(a, 0, "Unknown paging action: " + b);
			...
			if(g[a].bVisible != b) {
				if(b) {
					...
					for(f = j.length; e < f; e++)
						if(j[e].nTr !== null) k ? j[e].nTr.appendChild(j[e]._anHidden[a]) : j[e].nTr.insertBefore(j[e]._anHidden[a],
							T(d, e)[m])
				} else {
					e = 0;
					for(f = j.length; e < f; e++)
	...
	l.defaults = {
		...
		oLanguage: {
			...
			sInfo: "开始 _START_ 结束 _END_ 总共 _TOTAL_ 条数",
			sInfoEmpty: "开始 0 to 0 of 0 结束",
			sInfoFiltered: "(过滤从 _MAX_ 总共_TOTAL_  条数)",
			sInfoPostFix: "",
			sInfoThousands: ",",
			sLengthMenu: "展示条数 entries",
			sLoadingRecords: "进入中...",
			sProcessing: "处理中...",
			sSearch: "搜索:",
			sUrl: "",
